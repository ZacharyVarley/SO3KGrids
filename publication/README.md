...
`python -m publication.export_figures` produces the maintained paper figure set with ordered, content-descriptive stems from `publication/stems.py`.
...
- Line and text sizes constrained to the shared paper export defaults

## Output stems

Default output directory: `paper_figures/`
...
Adjust the shared paper typography defaults in `figures/common.py`, or tune the saved layouts in `figures/settings/` and rerun the exporter.
