...
from figures.common import (
    ...
    PAPER_LINE_LW_STD,
    PAPER_MARGINS,
    PAPER_MARKER_SIZE_STD,
    PAPER_PANEL_H,
    PAPER_TARGET_TEXT_MM,
    apply_paper_artwork_rcparams,
    apply_paper_typography,
    apply_paper_typography_full_page,
    clamp_paper_line_lw,
    export_paper_figure,
    paper_artwork_style,
    paper_font_sizes,
    paper_font_sizes_full_page,
    pt_to_mm,
    scale_scatter_point_size,
    validate_paper_fontsizes,
)
...
_PAPER_FIGURES_DIR = _ROOT_PATH / "paper_figures"
...
from publication import stems

_ARTWORK = paper_artwork_style()

_DEFAULT_OUT_DIR = str(_PAPER_FIGURES_DIR)
...
def _single_panel_axes(*, height: float = PAPER_PANEL_H):
    """One axes with the same margins as column interactive figures."""
    fig = plt.figure(figsize=(PAPER_COL_W, height))
    ax = fig.add_axes(
        [
            PAPER_MARGINS["left"],
            PAPER_MARGINS["bottom"],
            PAPER_MARGINS["right"] - PAPER_MARGINS["left"],
            PAPER_MARGINS["top"] - PAPER_MARGINS["bottom"],
        ]
    )
    return fig, ax
...
def _load_figure2_settings(path: str):
    ...
    apply_paper_typography(s)
    s.marker_size = PAPER_MARKER_SIZE_STD
    s.line_width = PAPER_LINE_LW_STD
    return s
...
def export_so3t_rejection_kr(out_dir: str) -> list[str]:
    ...
    s.fig_width = PAPER_FULL_W
    s.fig_height = PAPER_FULL_W * (_FIG1_REF_HEIGHT_IN / _FIG1_REF_WIDTH_IN)
    s.margin_left = 0.04
    s.margin_right = 0.90
    s.margin_top = 0.94
    ...
    apply_paper_typography_full_page(s, target_mm=PAPER_TARGET_TEXT_MM)
    s.point_size = scale_scatter_point_size(
        s.point_size,
        design_width_in=design_w,
        target_width_in=PAPER_FULL_W,
        reference_point_size=_FIG1_REF_POINT_SIZE,
        reference_width_in=_FIG1_REF_WIDTH_IN,
    )
    s.wire_thickness = clamp_paper_line_lw(s.wire_thickness, name="wire_thickness")
    print(
    ...
    with plt.rc_context({"text.usetex": True}):
        ...
        export_paper_figure(app.fig, stem, dpi=PAPER_DPI)
...
def export_nn_cdf_panels(out_dir: str) -> list[str]:
    ...
    apply_paper_typography(app.state)
    app.state.line_width = PAPER_LINE_LW_STD
...
def export_grid_method_comparison(out_dir: str) -> list[str]:
    ...
    with plt.rc_context({"text.usetex": True}):
        ...
        fig = plt.figure(figsize=(PAPER_COL_W, 1.05))
        ...
        fig = plt.figure(figsize=(PAPER_COL_W, 5.5), dpi=150)
        gs = GridSpec(
            3, 1,
...
def export_laue_o_pc_vs_fcc_kr(out_dir: str) -> list[str]:
    ...
    if os.path.exists(path):
        ...
        print(f"[settings] Loaded {path}")
    apply_paper_typography_full_page(s, field_map={"text_size": "label"})
    ...
    fig_h = PAPER_FULL_W * (_FIG6_REF_HEIGHT_IN / _FIG6_REF_WIDTH_IN)
    ui = f6.Figure7Interactive(
        row_top,
        row_bot,
        match,
        s,
        figsize=(PAPER_FULL_W, fig_h),
        dpi=150,
    ...
    with plt.rc_context({"text.usetex": True}):
        ...
        export_paper_figure(ui.fig, stem, dpi=PAPER_DPI)
        stems_out.append(stem)
        plt.close(ui.fig)
    return stems_out
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Export paper figures with descriptive stems (see publication/stems.py).",
    )
    parser.add_argument(
        "--only",
        type=int,
    ...
    apply_paper_artwork_rcparams()
    fonts = paper_font_sizes()
    fonts_full = paper_font_sizes_full_page()
    validate_paper_fontsizes(**fonts)
    validate_paper_fontsizes(**fonts_full)
    print(f"Column width: {PAPER_COL_W_MM:.1f} mm ({PAPER_COL_W:.3f} in)")
    print(
        f"Full-page width: {PAPER_FULL_W_MM:.1f} mm ({PAPER_FULL_W:.3f} in), "
        f"target label height {PAPER_TARGET_TEXT_MM:.1f} mm"
    )
    print(f"Export formats: {', '.join(PAPER_EXPORT_FORMATS)} @ {PAPER_DPI} d.p.i. (PNG)")
    print("Typography (printed height at column width):")
    for name, pt in sorted(fonts.items()):
        print(f"  {name}: {pt:.1f} pt → {pt_to_mm(pt):.2f} mm")
    print(
        f"Standard artwork: line {PAPER_LINE_LW_STD} pt, "
        f"markers {PAPER_MARKER_SIZE_STD} pt, marker edge {_ARTWORK['marker_edgewidth']} pt"
    )
    print("Stems: content-based names in publication/stems.py (ordered 01–12).")
