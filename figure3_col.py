...
DEFAULT_STEM = "figure3_col"
SETTINGS_FILE = "figure3_col_settings.json"
DPI = IUCR_DPI
...
def _resolve_self_nn_default() -> str:
    candidates = ["figure3_data_self_nn_col.npz", "figure3_data_self_nn.npz"]
    for candidate in candidates:
        if os.path.exists(candidate):
            return candidate
    return "figure3_data_self_nn_col.npz"


def main():
    parser = argparse.ArgumentParser(description="Figure 3 combined witness (left) + self-NN (right) CDF overlay")
    parser.add_argument("--witness-data", default="figure3_data_witness_col.npz", help="Path to witness-set NPZ data")
    ...
    if not os.path.exists(witness_path):
        ...
        if witness_path == "figure3_data_witness_col.npz" and os.path.exists("figure3_data_witness.npz"):
            witness_path = "figure3_data_witness.npz"
