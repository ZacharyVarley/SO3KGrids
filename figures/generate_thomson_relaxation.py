...
Usage:
        python -m figures.generate_thomson_relaxation              # generate data (if needed) + interactive plot
        python -m figures.generate_thomson_relaxation --force      # force-regenerate all data
        python -m figures.generate_thomson_relaxation --plot-only  # plot from existing .npz
        python -m figures.generate_thomson_relaxation --static     # non-interactive PNG/PDF export
        python -m figures.generate_thomson_relaxation --quick      # 5 iters (for testing)
        python -m figures.generate_thomson_relaxation --n-eff-min 50000 --n-eff-max 500000  # custom range
"""
...
DATA_FILE = str(_HERE / "data" / "figure5_thomson_relaxation.npz")
SETTINGS_FILE = str(_HERE / "settings" / "thomson_relaxation_generator.json")
SUMMARY_FILE = str(_HERE / "data" / "figure5_thomson_relaxation_summary.json")
DPI = 300
