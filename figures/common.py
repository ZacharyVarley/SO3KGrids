...
# ═══════════════════════════════════════════════════════════════════════════
#  Paper figure dimensions and standard defaults
# ═══════════════════════════════════════════════════════════════════════════

PAPER_COL_W_MM = 88.5   # single-column width (mm)
PAPER_COL_W   = PAPER_COL_W_MM / 25.4   # inches (≈ 3.48 in)
PAPER_FULL_W_MM = 171.0  # full-page width (mm)
PAPER_FULL_W  = PAPER_FULL_W_MM / 25.4  # inches (≈ 6.75 in)
PAPER_TARGET_TEXT_MM = 2.5  # target printed label height at native placement width
PAPER_MAX_H   = 9.50    # maximum figure height (inches)  — 241 mm
PAPER_PANEL_H = 2.75    # typical height for one split panel (inches)
PAPER_DPI     = 600     # monochrome line art target
PAPER_DPI_COLOR = 400   # colour raster target

# Line-weight band at final size (EPS/vector).
PAPER_LINE_LW_MIN = 0.35
PAPER_LINE_LW_MAX = 1.5
PAPER_LINE_LW_STD = 1.0
PAPER_MARKER_SIZE_STD = 4.0
PAPER_MARKER_EDGE_LW = PAPER_LINE_LW_MIN
PAPER_GRID_LW = PAPER_LINE_LW_MIN

# Preferred export formats; PDF is kept for local proofing.
PAPER_EXPORT_FORMATS = ("png", "eps", "pdf")

FIBONACCI_SERIES_KEYS = frozenset({"fib_rej", "fib_all"})

# Keep text height in a 1.5–3 mm band when the figure is placed at column width.
# Vector figures scale uniformly, so printed text height (mm) = fontsize (pt) × MM_PER_PT.
MM_PER_PT = 25.4 / 72.0
PAPER_TEXT_MM_MIN = 1.5
PAPER_TEXT_MM_MAX = 3.0
PAPER_TEXT_PT_MIN = PAPER_TEXT_MM_MIN / MM_PER_PT   # ≈ 4.25 pt
PAPER_TEXT_PT_MAX = PAPER_TEXT_MM_MAX / MM_PER_PT   # ≈ 8.47 pt
...
def clamp_paper_font_pt(pt: float, *, name: str = "font") -> float:
    """Clamp a font size to the 1.5–3 mm range at column width."""
    pt = float(pt)
    lo, hi = PAPER_TEXT_PT_MIN, PAPER_TEXT_PT_MAX
    if pt < lo:
        import warnings
        warnings.warn(f"{name}: {pt:.2f} pt ({pt_to_mm(pt):.2f} mm) below paper minimum; using {lo:.2f} pt")
        return lo
    if pt > hi:
        import warnings
        warnings.warn(f"{name}: {pt:.2f} pt ({pt_to_mm(pt):.2f} mm) above paper maximum; using {hi:.2f} pt")
        return hi
    return pt


def paper_font_sizes() -> dict[str, float]:
...
def paper_font_sizes_full_page(target_mm: float = PAPER_TARGET_TEXT_MM) -> dict[str, float]:
    """Font sizes (pt) for figures designed at full-page width (native ~2.5 mm labels)."""
    label = clamp_paper_font_pt(mm_to_pt(target_mm), name="label")
    tick = clamp_paper_font_pt(mm_to_pt(target_mm * 0.94), name="tick")
    legend = clamp_paper_font_pt(mm_to_pt(target_mm * 0.90), name="legend")
    legend_title = clamp_paper_font_pt(mm_to_pt(target_mm * 0.96), name="legend_title")
    annotation = clamp_paper_font_pt(mm_to_pt(target_mm * 0.94), name="annotation")
    minor_tick = clamp_paper_font_pt(mm_to_pt(target_mm * 0.82), name="minor_tick")
    return dict(
        label=label,
        tick=tick,
        legend=legend,
...
def clamp_paper_line_lw(lw: float, *, name: str = "line") -> float:
    """Clamp line width to the 0.35–1.5 pt band at final size."""
    lw = float(lw)
    lo, hi = PAPER_LINE_LW_MIN, PAPER_LINE_LW_MAX
    if lw < lo:
        import warnings
        warnings.warn(f"{name}: {lw:.2f} pt below paper minimum; using {lo:.2f} pt")
        return lo
    if lw > hi:
        import warnings
        warnings.warn(f"{name}: {lw:.2f} pt above paper maximum; using {hi:.2f} pt")
        return hi
    return lw


def paper_artwork_style() -> dict[str, float]:
    """Standard line and marker sizes for paper exports."""
    return dict(
        line_lw=PAPER_LINE_LW_STD,
        marker_size=PAPER_MARKER_SIZE_STD,
        marker_edgewidth=PAPER_MARKER_EDGE_LW,
        grid_lw=PAPER_GRID_LW,
    )
...
def apply_paper_artwork_rcparams() -> None:
    """Matplotlib rcParams aligned with the paper export workflow."""
...
def validate_paper_fontsizes(**named_pt: float) -> None:
    """Raise ValueError if any named size falls outside 1.5–3 mm at column width."""
    bad = []
    for name, pt in named_pt.items():
        mm = pt_to_mm(pt)
        if mm < PAPER_TEXT_MM_MIN or mm > PAPER_TEXT_MM_MAX:
            bad.append(f"{name}={pt:.2f} pt ({mm:.2f} mm)")
    if bad:
        raise ValueError(
            "Font sizes outside the 1.5–3 mm column-width band: " + ", ".join(bad)
        )


def apply_paper_typography_full_page(
    settings_obj,
    field_map: dict[str, str] | None = None,
    *,
    target_mm: float = PAPER_TARGET_TEXT_MM,
) -> dict[str, float]:
    """Override typography for figures placed at full-page width (~2.5 mm labels)."""
    fonts = paper_font_sizes_full_page(target_mm=target_mm)
...
def apply_paper_typography(
    ...
    *field_map* maps attribute names on *settings_obj* to keys in
    :func:`paper_font_sizes` (for example ``font_size`` → ``label``).
    """
...
def export_paper_figure(
    ...
    *,
    dpi: int = PAPER_DPI,
    formats: tuple[str, ...] = PAPER_EXPORT_FORMATS,
) -> None:
...
# Standard font sizes for paper single-column figures (pt).
# These stay legible after scaling to column width.
_F = paper_font_sizes()
PAPER_FONT = dict(
    label       = _F["label"],
    tick        = _F["tick"],
    title       = clamp_paper_font_pt(8.5, name="title"),
    subtitle    = _F["legend_title"],
    legend      = _F["legend"],
    legend_title= _F["legend_title"],
    panel_label = clamp_paper_font_pt(8.5, name="panel_label"),
    annotation  = _F["annotation"],
    cbar        = _F["tick"],
)

# Standard layout margins for paper single-column stacked panels.
PAPER_MARGINS = dict(
    left   = 0.18,
    right  = 0.97,
    top    = 0.97,
...
# Minimum line weight (pt) at final size.
PAPER_MIN_LW = PAPER_LINE_LW_MIN
