#!/usr/bin/env python3
"""
figure6_b.py — Primitive-cubic KR, FCC-KR, and KR FCC TC SRC for Laue O.

Same as figure6.py but adds a 3rd row for the new KR FCC TC SRC method.

The first two methods use cubochoric -> homochoric -> FZ (KR mapping).
The KR FCC TC SRC method maps directly from Euclidean truncated cube to the
FZ truncated cube (Rodrigues-Frank); it does not use cubochoric or homochoric.

Layout: 3 rows x 3 columns
  Row 0: Primitive cubic KR (cubochoric, homochoric, FZ)
  Row 1: FCC KR (cubochoric, homochoric, FZ)
  Row 2: KR FCC TC SRC (source=Euclidean TC, destination=FZ, colorbar in (2,2))
"""
...
from mappings.ho2hoFZ_O_FCC import (
    kr_grid_O_FCC,
    _cubochoric_fcc_grid,
    beta_fcc,
    CU_MAX as CU_MAX_FCC,
)


LAUE_O = 11
CARD_O = 24
SETTINGS_FILE = "figure6_b_settings.json"
DEFAULT_STEM = "figure6_b"
SETTINGS_VERSION = 2
...
# Row 2 uses different column labels (Euclidean TC -> FZ, no cubochoric/homochoric)
ROW2_COLUMN_LABELS = (
    r"$\mathrm{Source\;(Euclidean\;TC)}$",
    r"$\mathrm{Destination\;(FZ)}$",
)
...
class Settings:
    ...
    zoom_ho: float = 0.55
    zoom_fz: float = 1.35
    zoom_tc: float = 0.85  # Euclidean TC source (row 2 col 0)
    p_lo: float = 2.0
    p_hi: float = 98.0
...
def settings_from_raw(raw: dict, base: Settings | None = None) -> Settings:
    ...
    s.zoom_tc = float(np.clip(s.zoom_tc, 0.20, 1.60))
...
def n_tc_fcc(h: int) -> int:
    """Number of FCC points inside truncated cube for given h."""
    cu = _cubochoric_fcc_grid(h, torch.device("cpu"), torch.float64)
    beta = beta_fcc(h)
    tau = CU_MAX_FCC * (1.0 + beta)
...
def find_best_match(h_min: int, h_max: int, n_s3_min: int, n_s3_max: int) -> Match:
    ...
    for h in range(h_min, h_max + 1):
        ...
        n_tc = n_tc_fcc(h)
        n_s3_tc = n_s3_from_n_base(n_tc)
        if n_s3_min <= n_s3_tc <= n_s3_max:
            tcs.append((h, n_tc, n_s3_tc))

    if not pcs or not fccs:
        raise ValueError("No candidate h values found in the requested N_S^3 range.")
    if not tcs:
        raise ValueError("No KR FCC TC SRC sizes in the requested N_S^3 range.")
    ...
    for h_pc, n_pc, ns3_pc in pcs:
        for h_fcc, n_fcc, ns3_fcc in fccs:
            target_n_fz = (n_pc + n_fcc) // 2
            h_tc, n_tc, ns3_tc = min(tcs, key=lambda t: abs(t[1] - target_n_fz))
            diff_pf = abs(ns3_pc - ns3_fcc)
            rel_diff = diff_pf / max(ns3_pc, ns3_fcc)
            size_bias = abs(h_pc - h_fcc)
            tc_bias = abs(n_tc - target_n_fz)
            key = (rel_diff, diff_pf, tc_bias, size_bias, ns3_pc + ns3_fcc)
...
def _truncated_cube_euclidean_wire_segments(a: float, tau: float) -> list[np.ndarray]:
    """
    Truncated cube in Euclidean space: cube [-a,a]^3 ∩ {|x|+|y|+|z| <= tau}.

    This is the SOURCE domain for KR FCC TC SRC. The method maps directly from
    this Euclidean truncated cube to the FZ truncated cube (Rodrigues-Frank);
    it does not use cubochoric or homochoric as intermediates. The FZ wireframe
    in (c)/(f) is the truncated cube in Rodrigues-Frank space mapped to homochoric.
    """
    if tau <= a or tau >= 3 * a:
        return []

    # 24 vertices on the 12 cube edges (2 per edge)
    # Each edge (v1,v2) gets vertices from the two planes cutting the corners
    def edge_verts(c1, c2):
        # c1, c2 are cube corners. Plane at c: sign(c)·x = tau.
        c1, c2 = np.array(c1), np.array(c2)
        s1, s2 = np.sign(c1), np.sign(c2)
        diff = c2 - c1
        # v1 = c1 + t*(c2-c1) on plane dot(s1,v)=tau
        den1 = np.dot(s1, diff)
        t1 = (tau - np.dot(s1, c1)) / den1 if abs(den1) > 1e-12 else 0.0
        v1 = c1 + t1 * diff
        # v2 = c2 + t*(c1-c2) on plane dot(s2,v)=tau
        den2 = np.dot(s2, c1 - c2)
        t2 = (tau - np.dot(s2, c2)) / den2 if abs(den2) > 1e-12 else 0.0
        v2 = c2 + t2 * (c1 - c2)
        return v1, v2
    ...
    # 12 cube edges (each pair of corners that share an edge)
    edges_corners = [
        ((a, a, a), (a, a, -a)),
        ((a, a, a), (a, -a, a)),
        ((a, a, a), (-a, a, a)),
    ...
    verts_list = []
    for c1, c2 in edges_corners:
        v1, v2 = edge_verts(c1, c2)
        verts_list.append(v1)
        verts_list.append(v2)

    verts = np.unique(np.round(np.array(verts_list), decimals=10), axis=0)
    if len(verts) < 2:
        return []

    # Build edge segments: each original cube edge contributes one segment
    segments = []
    for c1, c2 in edges_corners:
        v1, v2 = edge_verts(c1, c2)
        segments.append(np.stack([v1, v2], axis=0))
    return segments
...
@torch.no_grad()
def build_row_fcc_tc_src(h: int, laue_id: int, device: torch.device):
    """Build row data for KR FCC TC SRC: source (cu_tc) and destination (ho_fz)."""
    qu_fz = kr_grid_O_FCC(h=h, device=device, return_type="qu")
    ho_fz = kr_grid_O_FCC(h=h, device=device, return_type="ho")

    cu_full = _cubochoric_fcc_grid(h, device, torch.float64)
    beta = beta_fcc(h)
    tau = CU_MAX_FCC * (1.0 + beta)
    keep = cu_full.abs().sum(dim=-1) <= (tau + 1e-12)
    cu_tc = cu_full[keep].detach().cpu().numpy()
    ...
    return {
        ...
        "ho_fz": ho_fz.detach().cpu().numpy(),
        "nn_fz": nn_fz,
        "n_fz": int(qu_fz.shape[0]),
        "tau": tau,
    }


class Figure6BInteractive:
    def __init__(
        self,
        row_top: dict,
        row_bot: dict,
        ...
        self.fz_extent = float(
            max(
                ...
                np.max(np.abs(self.row_tc["ho_fz"])),
            )
        )
        ...
        self.tc_wire_segments = _truncated_cube_euclidean_wire_segments(
            CU_MAX, float(self.row_tc["tau"])
        )

        self.fig = plt.figure(figsize=(14.6, 12.0), dpi=50, constrained_layout=True)
        ...
        self.axes = np.empty((3, 3), dtype=object)
        for r in range(3):
            for c in range(3):
                if r == 2 and c == 2:
                    self.cax = self.fig.add_subplot(gs[r, c])
                    self.axes[r, c] = self.cax
                else:
                    self.axes[r, c] = self.fig.add_subplot(gs[r, c], projection="3d")
                    self.axes[r, c].computed_zorder = False

        # Control panel — two columns, labels above sliders, wider window
        self.fig_ui = plt.figure("Figure 6b — Controls", figsize=(12.0, 7.5), dpi=110)
        self.fig_ui.patch.set_facecolor("#f8f9fa")
        self._colorbar = None
    ...
    def _build_ui(self):
        ...
        def _section(col: int, y_ref: float, title: str):
            L = L1 if col == 0 else L2
            W = CW
            ax = self.fig_ui.add_axes([L, y_ref, W, 0.018])
            ax.set_axis_off()
            ax.text(
                0,
                0.5,
        ...
        def _slider_with_label(
            ...
            W = CW
            # Label above
            ax_lbl = self.fig_ui.add_axes([L, y_ref, W, LH])
            ax_lbl.set_axis_off()
            ax_lbl.text(0, 0.5, label, fontsize=8.5, va="center")
            y_ref -= LH + 0.004
            # Slider
            ax = self.fig_ui.add_axes([L, y_ref - SH, W, SH])
            ax.set_facecolor("white")
            sl = Slider(ax, "", lo, hi, valinit=val, valstep=step)
            sl.label.set_visible(False)
            sl.valtext.set_fontsize(8)
            sl.valtext.set_ha("right")
            setattr(self, name_ax, ax)
            setattr(self, name_sl, sl)
            return y_ref - SH - DY

        def _textbox_with_label(
            col: int, y_ref: float, name_ax: str, name_tb: str, label: str, initial: str
        ):
            L = L1 if col == 0 else L2
            W = CW
            ax_lbl = self.fig_ui.add_axes([L, y_ref, W, LH])
            ...
            ax_lbl.text(0, 0.5, label, fontsize=8.5, va="center")
            y_ref -= LH + 0.004
            ax = self.fig_ui.add_axes([L, y_ref - BH, W, BH])
            tb = TextBox(ax, "", initial=initial)
            setattr(self, name_ax, ax)
            setattr(self, name_tb, tb)
            return y_ref - BH - 0.01

        def _buttons(col: int, y_ref: float, items: list[tuple[str, str]]):
            L = L1 if col == 0 else L2
            W = CW
            n = len(items)
            pad = 0.006
            ...
            for i, (attr, label) in enumerate(items):
                ax = self.fig_ui.add_axes([L + i * (bw + pad), y_ref - BH, bw, BH])
                b = Button(ax, label, color="white", hovercolor="#e9ecef")
        ...
        # Column 0
        y0 = 0.96
        y0 = _section(0, y0, "View")
        y0 = _slider_with_label(
            0, y0, "ax_elev", "sl_elev", "Elevation", -90.0, 90.0, self.s.elev, 1.0
        )
        y0 = _slider_with_label(
            0, y0, "ax_azim", "sl_azim", "Azimuth", -180.0, 180.0, self.s.azim, 1.0
        )
        y0 = _section(0, y0, "Display")
        y0 = _slider_with_label(
            0, y0, "ax_pt", "sl_pt", "Point size", 1.0, 50.0, self.s.point_size, 0.5
        )
        y0 = _slider_with_label(
            0,
            y0,
        ...
        y0 = _slider_with_label(
            0, y0, "ax_text", "sl_text", "Text size", 8.0, 20.0, self.s.text_size, 0.5
        )
        y0 = _section(0, y0, "Zoom")
        y0 = _slider_with_label(
            0,
            y0,
            "ax_zoom_cu",
        ...
        y0 = _slider_with_label(
            0, y0, "ax_zoom_fz", "sl_zoom_fz", "FZ", 0.30, 3.00, self.s.zoom_fz, 0.01
        )

        # Column 1
        y1 = 0.96
        y1 = _section(1, y1, "Colormap")
        y1 = _slider_with_label(
            1, y1, "ax_plo", "sl_plo", "Percentile low", 0.0, 30.0, self.s.p_lo, 0.5
        )
        y1 = _slider_with_label(
            1, y1, "ax_phi", "sl_phi", "Percentile high", 70.0, 100.0, self.s.p_hi, 0.5
        )
        y1 = _section(1, y1, "File I/O")
        y1 = _textbox_with_label(
            1,
            y1,
            "ax_settings_path",
            "tb_settings_path",
        ...
        y1 = _buttons(
            1, y1, [("bt_load", "Load"), ("bt_save", "Save"), ("bt_reset", "Reset")]
        )
        y1 = _textbox_with_label(
            1, y1, "ax_stem", "tb_stem", "Export stem", DEFAULT_STEM
        )
        y1 = _buttons(1, y1, [("bt_fig", "Save PNG/PDF")])
        ...
        for slider_name, slider in slider_map.items():
            slider.on_changed(lambda _v, n=slider_name: self._on_slider(n))

        self.bt_load.on_clicked(self._load_settings)
        self.bt_save.on_clicked(self._save_settings)
        self.bt_reset.on_clicked(self._reset_settings)
    ...
    def _sync_settings_from_widgets(self):
        ...
        self.s.zoom_tc = float(self.sl_zoom_tc.val)
    ...
    def _draw_row_tc(self, norm: Normalize):
        """Draw row 2: source (truncated cube) and destination (FZ)."""
        z_back = -1.0e6
        z_points = 0.0
        z_front = 1.0e6

        # Column 0: Source (Euclidean truncated cube)
        ax0 = self.axes[2, 0]
        lim_tc = max(CU_MAX * self.s.zoom_tc, float(self.row_tc["tau"]) * 1.1)
        lw_eff = 1.15 * self.s.wire_thickness

        wire_behind, wire_front = _split_wire_segments_by_view(
            self.tc_wire_segments, elev=self.s.elev, azim=self.s.azim
        )
        _draw_wire_segments_with_zorder(
            ax0, wire_behind, color="0.22", lw=lw_eff, zorder=z_back
        )
        _plot_points_depth_with_zorder(
            ax0,
            self.row_tc["cu_tc"],
        ...
        _draw_wire_segments_with_zorder(
            ax0, wire_front, color="0.22", lw=lw_eff, zorder=z_front
        )
        _style_common(ax0, lim_tc, self.s.elev, self.s.azim)
        ax0.set_axis_off()
        ax0.set_title("")

        # Column 1: Destination (FZ in homochoric)
        ax1 = self.axes[2, 1]
        lim_fz = self.fz_extent * self.s.zoom_fz
        lw_eff = 1.0 * self.s.wire_thickness

        wire_behind, wire_front = _split_wire_segments_by_view(
            self.fz_wire_ho, elev=self.s.elev, azim=self.s.azim
        )
        _draw_wire_segments_with_zorder(
            ax1, wire_behind, color="0.10", lw=lw_eff, zorder=z_back
        )
        _plot_points_depth_with_zorder(
            ax1,
            self.row_tc["ho_fz"],
        ...
        _draw_wire_segments_with_zorder(
            ax1, wire_front, color="0.10", lw=lw_eff, zorder=z_front
        )
        _style_common(ax1, lim_fz, self.s.elev, self.s.azim)
        ax1.set_axis_off()
        ax1.set_title("")

    def _draw_annotations(self, use_tex: bool = False):
        panel_letters = (
            np.array(
                [
        ...
        # Column labels for rows 0 and 1 only (Cubochoric, Homochoric, FZ)
        ...
        # Row 2: Source (Euclidean TC) and Destination (FZ) — TC SRC maps Euclidean TC → FZ
        # directly, without cubochoric or homochoric as intermediates
        if use_tex:
            row2_labels = (
                "$\\textrm{Source (Euclidean TC)}$",
                "$\\textrm{Destination (FZ)}$",
            )
        else:
            row2_labels = ROW2_COLUMN_LABELS
        for c, label in enumerate(row2_labels):
            self.axes[2, c].text2D(
                0.50,
                1.03,
                label,
                transform=self.axes[2, c].transAxes,
                fontsize=self.s.text_size,
                fontweight="bold",
                ha="center",
                va="bottom",
        ...
        if use_tex:
            row_labels = (
                ...
                "$\\textrm{KR FCC TC SRC}\\;(N_{\\mathrm{FZ}}="
                + str(self.row_tc["n_fz"])
                + ")$",
            )
        else:
            row_labels = (
                ...
                rf"$\mathrm{{KR\;FCC\;TC\;SRC}}\;(N_{{FZ}}={self.row_tc['n_fz']})$",
            )
    ...
    def _reset_settings(self, _event):
        """Reset all controls to default Settings values."""
        self.s = Settings()
        self._loading = True
        try:
            self._push_settings_to_widgets()
...
def main():
    ap = argparse.ArgumentParser(
        description="Figure 6b: primitive, FCC, and KR FCC TC SRC grid views for O"
    )
    ...
    ap.add_argument(
        "--save",
        action="store_true",
        help="Save figure6_b.png and figure6_b.pdf and exit",
    )
    ...
    print(
        f"Matched sizes: primitive h={match.h_pc} (N_FZ={match.n_pc:,}, N_S^3={match.n_s3_pc:,}) vs "
        f"fcc h={match.h_fcc} (N_FZ={match.n_fcc:,}, N_S^3={match.n_s3_fcc:,}), "
        f"rel_diff={100.0 * rel_diff:.3f}%"
    )
    print(
        f"KR FCC TC SRC: h={match.h_tc} (N_FZ={match.n_tc:,}, N_S^3={match.n_s3_tc:,})"
    )
    ...
    row_tc = build_row_fcc_tc_src(match.h_tc, LAUE_O, dev)
    ...
    ui = Figure6BInteractive(row_top, row_bot, row_tc, match, settings)
